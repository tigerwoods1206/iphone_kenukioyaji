...
#pragma mark top_layer
    top_layer = [CALayer layer];
    top_layer.frame=CGRectMake(0, 0, window_frame.size.width, window_frame.size.height);
    top_layer.backgroundColor = [UIColor clearColor].CGColor;
    //top_layer.backgroundColor = [UIColor colorWithPatternImage:backgroundImage].CGColor;
    top_layer.contents = (id)namihei.CGImage;
    top_layer.transform = CATransform3DMakeRotation(180 * M_PI /180, 0, 0, 1);
...
#pragma mark kenuki image load
   ...
   // top_layer.transform = CATransform3DMakeRotation(180 * M_PI / 180, 0.0, 0.0, 1);
...
#pragma mark init parent kenuki
    ...
    [top_layer addSublayer:parent_kenuki_layer];
...
#pragma mark init particle
    ...
    [top_layer addSublayer:emitterLayer];
...
#pragma mark init skin
    ...
    [top_layer addSublayer:skin_layer];
    ...
    [top_layer addSublayer:hit_namihei_face_layer];
...
#pragma mark init koukoku
   ...
   // ADBannerViewを画面下側のちょうど隠れる位置に移動
    ...
    adView.frame = [top_layer convertRect:adView.frame fromLayer:self.view.layer];
...
#pragma mark set Label (Score)
    ...
    scoreLabel.frame = [top_layer convertRect:scoreLabel.frame fromLayer:self.view.layer];
    scoreLabel.text = [NSString stringWithFormat:@"%d Pt",_Score];   //@"0000";
...
#pragma mark set animation Label
    ...
    comboLabel.frame = [top_layer convertRect:comboLabel.frame fromLayer:self.view.layer];
    comboLabel.text = @"";
...
#pragma mark set timer
    ...
    self.timeLabel.center = CGPointMake(self.timeLabel.frame.size.width/2,
                                        screenRect.size.height - adView.frame.size.height -
                                        scoreLabel.frame.size.height - self.timeLabel.frame.size.height/2);
    self.timeLabel.center = [top_layer convertPoint:self.timeLabel.center fromLayer:self.view.layer];
    self.timeLabel.text = [NSString stringWithFormat:@"残り %2d",Time];
    //self.timeLabel.transform = CGAffineTransformMakeRotation(180 * M_PI /180);
...
#pragma mark hige restart touch namihei
        ...
        Layer *kenuki_parent = [top_layer.sublayers objectAtIndex:0];
...
//        printf("Fisrt moving Touch ");
        ...
        CGPoint first_pt     = [first_obj locationInView:self.view];
        first_pt = [top_layer convertPoint:first_pt fromLayer:self.view.layer];
        CGPoint second_pt    = [second_obj locationInView:self.view];
        second_pt = [top_layer convertPoint:second_pt fromLayer:self.view.layer];
        Layer *kenuki_parent = [top_layer.sublayers objectAtIndex:0];
        Layer *lay1 = [kenuki_parent.sublayers objectAtIndex:0];
        Layer *lay2 = [kenuki_parent.sublayers objectAtIndex:1];
        ...
        if (first_Touch_Flg) {
            ...
            lay1.pre_pt = [top_layer convertPoint:lay1.pre_pt fromLayer:self.view.layer];
            lay2.pre_pt = [top_layer convertPoint:lay2.pre_pt fromLayer:self.view.layer];
        }
        ...
        if ( 
            ...
            CGRect hit_rect1 = [hit_sub1 convertRect:hit_sub1.frame toLayer:top_layer];
            CGRect hit_rect2 = [hit_sub2 convertRect:hit_sub2.frame toLayer:top_layer];
           ...
           if( [lay1 is_intersect_circlelay:hit_namihei_face_layer layer2:hit_sub2]) {
            ...
            hit_rect1 = [hit_sub1 convertRect:hit_sub1.frame toLayer:top_layer];
            hit_rect2 = [hit_sub2 convertRect:hit_sub2.frame toLayer:top_layer];
...
#pragma mark rotate parent kenuki
            ...
            if ([in_window_wall_judge is_in_window:lay1
                                      window_frame: [[UIScreen mainScreen] bounds] top_layer:top_layer]) {
                NSLog(@"lay1 out");
                collision_out = TRUE;
            }
            ...
            else if ([is_collision_face_judge reflection:lay1 face:hit_namihei_face_layer top_layer:top_layer]) {
                collision_out = TRUE;
                NSLog(@"hit face");
            }
            else if ([is_collision_face_judge reflection:lay2 face:hit_namihei_face_layer top_layer:top_layer]) {
                collision_out = TRUE;
                NSLog(@"hit face");
            }
            else {
...
//                    }
            ...
            for (higeLayer *hige in skin_layer.sublayers) {
                ...
                self.animation_Score.center = [top_layer convertPoint:self.animation_Score.center fromLayer:self.view.layer];
                comboLabel.center = CGPointMake(hige.frame.origin.x, hige.frame.origin.y + 50);
                comboLabel.center = [top_layer convertPoint:comboLabel.center fromLayer:self.view.layer];
                if (hige.skinout_flg && !ite_draw) {
                    #pragma mark change namihei ite

                    top_layer.contents = (id)namihei_ite.CGImage;
                    ite_draw = TRUE;
...
//    UIGraphicsEndImageContext();
  //  self.view.backgroundColor = [UIColor colorWithPatternImage:backgroundImage];
    ...
    Layer *parent_kenuki_layer = [top_layer.sublayers objectAtIndex:0];
...
-(void)setScore:(int)addscore
{
    ...
    _Score += addscore;
    scoreLabel.text = [NSString stringWithFormat:@"%d Pt", _Score];
...
-(void)countDown
{
    ...
    if (Time < 0) {
        self.timeout_view = [[TimeoutView alloc] initWithNibName:@"TimeoutView" bundle:[NSBundle mainBundle]];
        Time = 0;
        [self presentModalViewController:self.timeout_view animated:YES];
    }
}

#pragma mark save userdefaultfile
-(void)saveUserDefault
{
    // save userdefaylt file
    NSUserDefaults* defaults = [NSUserDefaults standardUserDefaults];
    [defaults setValue:[NSNumber numberWithInt:_Score] forKey:@"Score"];
